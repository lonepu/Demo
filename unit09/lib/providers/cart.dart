...
class Cart with ChangeNotifier {
  ...
  void removeSingleItem(String productId) {
    // quantity တစ်ခုလျှော့မယ့် method
    if (!_items.containsKey(productId)) {
      return; // အကယ်၍ item မရှိရင် ဘာမှ မလုပ်ဘူး
    }
    if (_items[productId]!.quantity > 1) {
      // quantity က 1 ထက်ကြီးရင် quantity ကို တစ်ခုလျှော့
      _items.update(
        productId,
        (existingCartItem) => CartItem(
          id: existingCartItem.id,
          title: existingCartItem.title,
          price: existingCartItem.price,
          quantity: existingCartItem.quantity - 1, // quantity တစ်ခုလျှော့
        ),
      );
    } else {
      // quantity က 1 ဖြစ်ရင် အဲ့ဒီ item ကို ဖယ်ရှား
